...
public final class BaseUtil {
    ...
    public static <T> T getObject(BaseResult result, Class<T> clazz) {
        if (!result.isSuccess()) return null;
        return JSONObject.parseObject(JSON.toJSONString(result.getRespData()), clazz);
    }

    public static <T> List<T> getList(BaseResult result, Class<T> clazz) {
        if (!result.isSuccess()) return Collections.emptyList();
        return JSONObject.parseArray(JSON.toJSONString(result.getRespData()), clazz);
    }

    public static <T> T getObjectResult(BaseResult result, Class<T> clazz) {
        if (!result.isSuccess()) throw new BaseException(result);
        return JSONObject.parseObject(JSON.toJSONString(result.getRespData()), clazz);
    }

    public static <T> List<T> getListResult(BaseResult result, Class<T> clazz) {
        if (!result.isSuccess()) throw new BaseException(result);
        return JSONObject.parseArray(JSON.toJSONString(result.getRespData()), clazz);
    }
